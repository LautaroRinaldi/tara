...
Camera::Camera(rclcpp::Node::SharedPtr _comm_nh, rclcpp::Node::SharedPtr _param_nh) :
	...
	void Camera::callBackExposure (std_msgs::msg::Float64 call_exposure_msg)
	{
	...
	void Camera::callBackBrightness (std_msgs::msg::Float64 call_brightness_msg)
	{
    ...
    void Camera::sendInfo(ImagePtr &image, rclcpp::Time time) {
    ...
    void Camera::timeCb(builtin_interfaces::msg::Time time)
    {
      time_mutex_.lock();
    	last_time = time.data;
    ...
    void Camera::feedImages() {
      ...
      while (ok) {
        ...
        time_mutex_.lock();
        rclcpp::Time capture_time = last_time;
        time_mutex_.unlock();
//        rclcpp::Time capture_time = last_time;
